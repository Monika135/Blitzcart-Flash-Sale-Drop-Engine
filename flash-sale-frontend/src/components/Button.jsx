export default function Button({
  ...
  const base = 'rounded-xl font-body font-semibold text-sm transition-all duration-200 active:scale-[0.98] flex items-center justify-center gap-2.5 disabled:active:scale-100 disabled:cursor-not-allowed';
  const variants = {
  ...
  const defaultWidth = /\bw-/.test(className) ? '' : 'w-full';
  const defaultHeight = /\bh-/.test(className) || /\bpy-/.test(className) ? '' : 'h-12';
  const defaultPadding = /\bpx-/.test(className) ? '' : 'px-4';
  ...
  return (
    <button
      ...
      aria-busy={loading}
      className={`${base} ${variants[variant]} ${defaultWidth} ${defaultHeight} ${defaultPadding} ${className}`.trim()}
    >
      {loading && (
